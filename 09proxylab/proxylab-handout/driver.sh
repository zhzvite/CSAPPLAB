...
#综上所述，这行命令的意思是：当脚本接收到ALRM信号时，打印一条消息“Timeout waiting for 
#the server to grab the port reserved for it”，然后杀死当前脚本的进程。这通常用于处理超时情况，
#例如，当脚本等待服务器占用某个端口时，如果等待时间过长，就通过发送ALRM信号来终止脚本的执行。
#####
# Basic
#
echo "*** Basic ***"
...
# 先启动tiny，获取到tiny——port后再启动proxy_port
# Run the proxy
